...
class CSVLogger:
    ...
    def _initialize_csv_files(self):
        ...
        clips_headers = [
            ...
            'channel_1_status', 'channel_1_video_id', 'channel_1_url',
            'channel_2_status', 'channel_2_video_id', 'channel_2_url',
            'channel_3_status', 'channel_3_video_id', 'channel_3_url',
    ...
    def log_clip(self, clip: VideoClip, upload_results: Dict[str, Dict]):
        ...
        with open(self.clips_csv, 'a', newline='', encoding='utf-8') as f:
            ...
            writer.writerow([
                ...
                clip.created_at,
                channel_data['channel_1_status'], channel_data['channel_1_video_id'], channel_data['channel_1_url'],
                channel_data['channel_2_status'], channel_data['channel_2_video_id'], channel_data['channel_2_url'],
                channel_data['channel_3_status'], channel_data['channel_3_video_id'], channel_data['channel_3_url'],
                total_published, total_failed
            ])

class MultiChannelYouTubeManager:
    """Handles YouTube API operations for multiple channels"""
    ...
    def _authenticate_all_channels(self):
        ...
        for i, channel_config in enumerate(self.channel_configs):
            ...
            if not credentials_file or not os.path.exists(credentials_file):
                logger.warning(f"Credentials file not found for {channel_key}: {credentials_file}")
                continue
            ...
            try:
                ...
                if service:
                    ...
                    logger.info(f"✅ Authenticated {channel_key}: {channel_config['name']}")
                else:
                    logger.error(f"❌ Failed to authenticate {channel_key}")
    ...
    def upload_to_all_channels(self, file_path: str, title: str, description: str, 
                              tags: List[str], delay_between_uploads: int = 30) -> Dict[str, Dict]:
        """Upload video to all configured channels with delays"""
        results = {}
        
        for channel_key, channel_data in self.youtube_services.items():
            try:
                logger.info(f"Uploading to {channel_key}: {channel_data['config']['name']}")
                
                result = self._upload_to_channel(
                    channel_data['service'], file_path, title, description, tags
                )
                
                if result['status'] == 'success':
                    video_url = f"https://www.youtube.com/watch?v={result['video_id']}"
                    result['url'] = video_url
                    logger.info(f"✅ Successfully uploaded to {channel_key}: {video_url}")
                else:
                    logger.error(f"❌ Failed to upload to {channel_key}: {result['error']}")
                
                results[channel_key] = result
                
                # Wait between uploads to avoid rate limiting
                if len(results) < len(self.youtube_services):
                    logger.info(f"Waiting {delay_between_uploads} seconds before next upload...")
                    time.sleep(delay_between_uploads)
                
            except Exception as e:
                logger.error(f"❌ Error uploading to {channel_key}: {e}")
                results[channel_key] = {
                    'status': 'failed',
                    'error': str(e),
                    'video_id': '',
    ...
    def _upload_to_channel(self, service, file_path: str, title: str, description: str, 
                          tags: List[str], category_id: str = '27') -> Dict:
...
class MIHContentAutomation:
        ...
        for clip_index, clip in enumerate(clips):
            ...
            try:
                ...
                upload_results = self.youtube_manager.upload_to_all_channels(
                    ...
                    tags=clip.hashtags,
                    delay_between_uploads=30  # 30 seconds between channel uploads
                )
                ...
                # Log failed clip
                failed_results = {f'channel_{i+1}': {'status': 'failed', 'error': str(e), 'video_id': '', 'url': ''} 
            ...
            # Wait between clip uploads (30 seconds to 1 minute)
            if clip_index < len(clips) - 1:
                wait_time = 45  # 45 seconds between clips
                logger.info(f"⏱️  Waiting {wait_time} seconds before publishing next clip...")
    ...
    def run_automation_cycle(self):
        ...
        total_clips = 0
        processed_count = 0
        max_videos_per_cycle = 3  # Process fewer videos per cycle for better control
        ...
        for video_index, video_data in enumerate(videos[:max_videos_per_cycle]):
            ...
            try:
                ...
                if video_index < min(len(videos), max_videos_per_cycle) - 1:
                    wait_time = 120  # 2 minutes between videos
                    logger.info(f"⏱️  Waiting {wait_time} seconds before processing next video...")
                    time.sleep(wait_time)
...
def main():
    ...
    try:
        ...
        # print("abc ",automation_config.UPLOAD_CHANNELS)
    except ImportError:

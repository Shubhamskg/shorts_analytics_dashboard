...
class AlternativeTranscriptProcessor:
    ...
    def get_transcript_whisper(self, video_id: str) -> List[Dict]:
        """Method 2: Use OpenAI Whisper for transcription (more reliable)"""
        try:
            logger.info(f"Trying Whisper for {video_id}")
            ...
            # Download audio only
            audio_file = self.temp_dir / f"{video_id}.wav"
            
            cmd = [
                'yt-dlp',
                '-x', '--audio-format', 'wav',
                '--audio-quality', '0',
                '-o', str(audio_file).replace('.wav', '.%(ext)s'),
                f'https://www.youtube.com/watch?v={video_id}'
            ]
            ...
            for ext in ['.wav', '.m4a', '.mp3']:
                test_file = str(audio_file).replace('.wav', ext)
            ...
            json_file = self.temp_dir / f"{video_id}.json"
            if os.path.exists(json_file):
                ...
                os.remove(json_file)
                logger.info(f"Whisper successful: {len(transcript)} segments")
                return transcript
            
            return []
            
        except Exception as e:
            logger.error(f"Whisper method failed: {e}")
            return []
...
class TranscriptProcessor:
    ...
    def get_transcript(self, video_id: str) -> List[Dict]:
        """Get transcript using multiple methods (avoids timedtext rate limits)"""
        
        # Method priority (fastest to most reliable)
        methods = [
            ("yt-dlp auto-subs", self.alternative_processor.get_transcript_yt_dlp),
            # ("Whisper", self.alternative_processor.get_transcript_whisper),
        ]
        
        logger.info(f"Getting transcript for {video_id} using alternative methods")
        ...
        for method_name, method_func in methods:
            try:
                ...
                if transcript and len(transcript) > 0:
                    logger.info(f"✅ {method_name} succeeded with {len(transcript)} segments")
                    return transcript
                else:
...
class MIHContentAutomation:
    ...
    def publish_clips(self, clips: List[VideoClip]):
        """Generate content and publish clips to YouTube channels one by one"""
        if not clips:
            logger.info("No clips to publish")
            return
        
        logger.info(f"Publishing {len(clips)} clips sequentially...")
        ...
        for clip_index, clip in enumerate(clips):
            ...
            logger.info(f"Publishing clip {clip_index + 1}/{len(clips)}: {os.path.basename(clip.file_path)}")
            ...
            try:
                ...
                for channel_index, channel_config in enumerate(self.upload_channels):
                    try:
                        logger.info(f"Uploading to channel {channel_index + 1}: {channel_config.get('name', 'Unknown')}")
                        ...
                        if video_id:
                            logger.info(f"Successfully uploaded clip to channel {channel_index + 1}: {video_id}")
                            upload_successful = True
                        else:
                            logger.error(f"Failed to upload to channel {channel_index + 1}")
                        
                        # Short delay between channel uploads
                        if channel_index < len(self.upload_channels) - 1:
                            logger.info("Waiting 10 seconds before next channel...")
                            time.sleep(10)
                        
                    except Exception as e:
                        logger.error(f"Error uploading to channel {channel_index + 1}: {e}")
                
                if upload_successful:
                    logger.info(f"Clip {clip_index + 1} published successfully")
                else:
                    logger.error(f"Failed to upload clip {clip_index + 1} to any channel")
            ...
            # Wait 1 minute between clip uploads (except for the last clip)
            if clip_index < len(clips) - 1:
                logger.info("Waiting 1 minute before publishing next clip...")
                time.sleep(60)
        
        logger.info(f"Completed publishing all {len(clips)} clips")
    
    def run_automation_cycle(self):
        """Run one complete automation cycle - process videos one by one"""
        ...
        total_clips = 0
        processed_count = 0
        max_videos_per_cycle = 5  # Process fewer videos per cycle for better control
        ...
        for video_index, video_data in enumerate(videos[:max_videos_per_cycle]):
            ...
            try:
                ...
                logger.info(f"Step 2: Publishing {len(clips)} clips sequentially...")
                ...
                if video_index < min(len(videos), max_videos_per_cycle) - 1:
                    logger.info("⏱️  Waiting 2 minutes before processing next video...")
                    time.sleep(120)  # 2 minute gap between videos

...
# pip install google-generativeai google-api-python-client google-auth-httplib2 google-auth-oauthlib gTTS pytrends yt-dlp requests

# Setup Instructions:
# 1. Install Python 3.8+
# 2. Install requirements above
# 3. Install FFmpeg and add to PATH
# 4. Get YouTube Data API v3 key
# 5. Get Google Gemini API key
# 6. Setup OAuth credentials for YouTube upload
# 7. Configure config.py with your settings
# 8. Run: python mih_automation.py --video VIDEO_IDimport os
import re
import json
import time
import logging
...
class EnhancedContentGenerator:
    ...
    def find_viral_clips(self, transcript: str, video_data: Dict, trending_topics: List[TrendingTopic]) -> List[Dict]:
        ...
        prompt = f"""
        You are analyzing a video featuring Dr. Linda Greenwall, a globally recognized expert in Molar Incisor Hypomineralisation (MIH) and pediatric whitening.

        VIDEO CONTEXT:
        Title: "{video_data.get('title', '')}"
        Transcript: "{transcript}"

        TRENDING TOPICS TO PRIORITIZE:
        {trending_context}

        TASK: Identify the 3 most VIRAL-WORTHY clips (15-90 seconds each) that will:
        1. GRAB ATTENTION of worried parents in the first 3 seconds
        2. Provide ACTIONABLE ADVICE about MIH/dental health
        3. Feature Dr. Greenwall's EXPERTISE prominently
        4. Match TRENDING search topics when possible
        ...
        [
            {{
                "start_timestamp": 45,
                "end_timestamp": 75,
                "viral_hook": "The shocking reason your child's teeth have white spots",
                "parent_appeal": "High - addresses common MIH concern",
                "trending_match": "chalky teeth children",
                "key_takeaway": "Parents can identify MIH early with this simple check"
            }}
        ]
        """
        
        try:
            logger.info("AI analyzing transcript for viral clips")
    ...
    def generate_viral_content(self, transcript: str, duration: float, trending_topics: List[TrendingTopic], clip_data: Dict = None) -> Dict:
        ...
        trending_context = self._build_trending_context(trending_topics[:3])
        viral_hook = clip_data.get('viral_hook', '') if clip_data else ''
        key_takeaway = clip_data.get('key_takeaway', '') if clip_data else ''
        
        prompt = f"""
        Create VIRAL social media content for a {duration:.0f}-second clip featuring Dr. Linda Greenwall, MIH expert.
        ...
        TARGET AUDIENCE: Worried parents of children with MIH/dental issues

        Return ONLY this JSON format:
        {{
            "title": "The MIH Secret Dentists Don't Tell Parents",
            "description": "Dr. Greenwall reveals what those white spots really mean! Don't wait - early action saves your child's smile #ParentAlert",
            "hashtags": ["#MIH", "#PediatricDentistry", "#DrGreenwall"]
        }}
        """
        
        try:
            logger.info("AI generating viral content")
            response = self.model.generate_content(prompt)
            json_match = re.search(r'\{.*?\}', response.text, re.DOTALL)
            if json_match:
...
class EnhancedVisualCreator:
    ...
    def create_branded_intro(self, title: str, trending_hook: str = "", duration: float = 4.0) -> Optional[str]:
        output_path = self.temp_dir / f"intro_{uuid.uuid4().hex[:8]}.mp4"
        logger.info(f"Creating branded intro: {title[:30]}...")
        
        intro_text = trending_hook if trending_hook else f"Dr. Greenwall reveals: {title}"
        tts_file = self._create_tts_audio(intro_text)
        ...
        cmd = [
            'ffmpeg', '-y', '-v', 'error',
            '-f', 'lavfi', '-i', f'color=c={self.config.brand_colors["bg_gradient_start"]}:s=1080x1920:d={duration}',
            '-i', tts_file
        ]
        ...
        # Fixed video filters - simplified and working
        video_filters = [
            'format=rgb24',
            'geq=r=120:g=87:b=196',  # Simple static gradient
            'zoompan=z=1.05:d=1:x=iw/2-(iw/zoom/2):y=ih/2-(ih/zoom/2):s=1080x1920',
            f"drawtext=text='{self._escape_text_for_ffmpeg(title)}':fontsize=30:fontcolor=white:x=(w-text_w)/2:y=(h-text_h)/2-100:box=1:boxcolor=black@0.5",
            f"drawtext=text='{self.config.expert_name}':fontsize=40:fontcolor=white:x=(w-text_w)/2:y=(h-text_h)/2+50:box=1:boxcolor=black@0.5",
            f"drawtext=text='MIH Expert':fontsize=28:fontcolor=white:x=(w-text_w)/2:y=(h-text_h)/2+120:box=1:boxcolor=black@0.5",
            'setsar=1'
        ]
        ...
        try:
            result = self._run_ffmpeg_command(cmd, timeout=60)
            if result and output_path.exists() and output_path.stat().st_size > 1000:
                logger.info(f"Intro created successfully: {output_path.name}")
                return str(output_path)
            else:
                logger.error("Intro creation failed - output file missing or too small")
        except Exception as e:
    ...
    def create_branded_outro(self, duration: float = 3.5) -> Optional[str]:
        ...
        cta_text = "Follow for more MIH expert tips!"
        tts_file = self._create_tts_audio(cta_text)
        
        cmd = [
            'ffmpeg', '-y', '-v', 'error',
            '-f', 'lavfi', '-i', f'color=c={self.config.brand_colors["bg_gradient_end"]}:s=1080x1920:d={duration}',
            '-i', tts_file
        ]
        ...
        video_filters = [
            'format=rgb24',
            'geq=r=255:g=107:b=107',  # Simple coral color
            f"drawtext=text='FOLLOW FOR MORE':fontsize=55:fontcolor=white:x=(w-text_w)/2:y=(h-text_h)/2-80:box=1:boxcolor=black@0.5",
            f"drawtext=text='{self.config.expert_handle}':fontsize=45:fontcolor=white:x=(w-text_w)/2:y=(h-text_h)/2-20:box=1:boxcolor=black@0.5",
            f"drawtext=text='MIH Expert Tips':fontsize=35:fontcolor=white:x=(w-text_w)/2:y=(h-text_h)/2+40:box=1:boxcolor=black@0.5",
            f"drawtext=text='LIKE - SUBSCRIBE - NOTIFY':fontsize=32:fontcolor=white:x=(w-text_w)/2:y=(h-text_h)/2+100:box=1:boxcolor=black@0.5",
            'setsar=1'
        ]
        ...
        try:
            result = self._run_ffmpeg_command(cmd, timeout=45)
            if result and output_path.exists() and output_path.stat().st_size > 1000:
                logger.info(f"Outro created successfully: {output_path.name}")
                return str(output_path)
            else:
                logger.error("Outro creation failed - output file missing or too small")
        except Exception as e:
    ...
    def _create_tts_audio(self, text: str, lang: str = 'en') -> Optional[str]:
        try:
            ...
            if tts_file.exists() and tts_file.stat().st_size > 100:
                logger.debug(f"TTS audio created: {clean_text[:30]}...")
                return str(tts_file)
        except Exception as e:
            logger.error(f"TTS audio creation failed: {e}")
        return None
    
    # Remove these problematic functions and replace with simple ones:

    def _create_animated_text_filter(self, text: str, fontsize: int, y: str, color: str, animation: str = 'fade') -> str:
        # Simplified - just return basic drawtext
        safe_text = self._escape_text_for_ffmpeg(text)
        return f"drawtext=text='{safe_text}':fontsize={fontsize}:fontcolor={color}:x=(w-text_w)/2:y={y}:box=1:boxcolor=black@0.5"

    def _create_text_filter(self, text: str, fontsize: int, y: str, color: str) -> str:
        # Simplified - basic drawtext without fontweight
        safe_text = self._escape_text_for_ffmpeg(text)
        return f"drawtext=text='{safe_text}':fontsize={fontsize}:fontcolor={color}:x=(w-text_w)/2:y={y}:box=1:boxcolor=black@0.5"
    ...
    def _run_ffmpeg_command(self, cmd: List[str], timeout: int = 120) -> bool:
        try:
            ...
            if result.returncode != 0:
                logger.error(f"FFmpeg command failed with code {result.returncode}: {result.stderr[:200]}")
                return False
...
class TranscriptProcessor:
    def __init__(self):
        ...
        self.temp_dir.mkdir(exist_ok=True, parents=True)
        logger.info(f"Transcript processor initialized with temp dir: {self.temp_dir}")
    
    def get_enhanced_transcript(self, video_id: str) -> List[Dict]:
        ...
        try:
            ...
            if not srt_files:
                logger.error(f"No subtitle files found for {video_id}")
                return []
            
            manual_subs = [f for f in srt_files if 'auto' not in f.name.lower()]
            best_sub_file = manual_subs[0] if manual_subs else srt_files[0]
            
            logger.info(f"Using subtitle file: {best_sub_file.name}")
            transcript = self._parse_enhanced_srt(best_sub_file)
            
            if transcript:
                transcript = self._enhance_transcript_for_mih(transcript)
    ...
    def _parse_enhanced_srt(self, srt_file: Path) -> List[Dict]:
        transcript = []
        ...
        try:
            ...
            if not content:
                logger.error(f"Could not decode subtitle file: {srt_file.name}")
                return []
    ...
    def _parse_srt_timestamp(self, timestamp: str) -> float:
        ...
        time_match = re.match(r'(\d{1,2}):(\d{2}):(\d{2})\.(\d{3})', timestamp)
        if not time_match:
    ...
    def _enhance_transcript_for_mih(self, transcript: List[Dict]) -> List[Dict]:
        enhanced_transcript = []
        
        for segment in transcript:
            segment['mih_relevance'] = self._calculate_mih_relevance(segment['text'])
            segment['emotional_impact'] = self._calculate_emotional_impact(segment['text'])
            segment['parent_appeal'] = self._calculate_parent_appeal(segment['text'])
            enhanced_transcript.append(segment)
        
        return enhanced_transcript
    
    def _calculate_mih_relevance(self, text: str) -> float:
        mih_keywords = [
            'MIH', 'molar', 'incisor', 'hypomineralisation', 'enamel', 'defect',
            'chalky', 'white spots', 'discoloration', 'ICON', 'treatment',
            'pediatric', 'children', 'kids', 'whitening', 'dental'
        ]
        
        text_lower = text.lower()
        matches = sum(1 for keyword in mih_keywords if keyword.lower() in text_lower)
        
        if 'greenwall' in text_lower or 'dr' in text_lower:
            matches += 2
        
        return min(matches / len(mih_keywords), 1.0)
    
    def _calculate_emotional_impact(self, text: str) -> float:
        high_impact_words = [
            'warning', 'danger', 'urgent', 'important', 'shocking', 'surprising',
            'never', 'always', 'must', 'should', 'worried', 'concerned',
            'mistake', 'wrong', 'right', 'secret', 'truth', 'reveal'
        ]
        
        text_lower = text.lower()
        impact_score = sum(1 for word in high_impact_words if word in text_lower)
    ...
    def _calculate_parent_appeal(self, text: str) -> float:
        parent_keywords = [
            'parent', 'parents', 'child', 'children', 'kids', 'son', 'daughter',
            'family', 'home', 'school', 'worry', 'concerned', 'help', 'advice',
            'tip', 'guide', 'what to do', 'how to', 'when to', 'should I'
        ]
        
        text_lower = text.lower()
        appeal_score = sum(1 for keyword in parent_keywords if keyword in text_lower)
        
        action_words = ['do', 'try', 'avoid', 'start', 'stop', 'check', 'look for']
        appeal_score += sum(0.5 for word in action_words if word in text_lower)
        
        return min(appeal_score / 3.0, 1.0)
    
    def cleanup(self):
        try:
            if self.temp_dir.exists():
                shutil.rmtree(self.temp_dir, ignore_errors=True)
                logger.info("Transcript processor temporary files cleaned up")
        except Exception as e:
            logger.warning(f"Transcript cleanup warning: {e}")
...
class EnhancedVideoProcessor:
    ...
    def _create_enhanced_main_clip(self, source_file: str, start: float, end: float, subtitles: List[Dict], content_data: Dict) -> Optional[str]:
        ...
        video_filters = [
            ...
            f"drawtext=text='{self.config.expert_handle}':fontcolor=white@0.7:fontsize=28:x=w-tw-25:y=25:box=1:boxcolor=black@0.4:boxborderw=5",
            f"drawtext=text='MIH EXPERT':fontcolor={self.config.brand_colors['accent']}:fontsize=24:x=25:y=25:box=1:boxcolor=black@0.6:boxborderw=3"
        ]
        
        if subtitle_file:
            subtitle_path = self.visual_creator._escape_path_for_ffmpeg(subtitle_file)
            # Updated subtitle style with font size 12
            subtitle_style = (
                "FontName=Arial Black,FontSize=12,PrimaryColour=&HFFFFFF,SecondaryColour=&HFFFF00,"
                "BackColour=&H80000000,BorderStyle=1,Outline=2,Shadow=1,Alignment=2,MarginV=120,"
                "Bold=1"
            )
            video_filters.append(f"subtitles='{subtitle_path}':force_style='{subtitle_style}'")
            logger.info("Subtitles will be embedded with font size 12")
        else:
            logger.warning("No subtitles available for main clip")
        ...
        try:
            result = self.visual_creator._run_ffmpeg_command(cmd, timeout=300)
            
            if result and output_path.exists() and output_path.stat().st_size > 1000:
                logger.info(f"Main clip created successfully: {output_path.name}")
                return str(output_path)
            else:
                logger.error("Main clip creation failed - output file missing or too small")
                return None
        except Exception as e:
    ...
    def _concatenate_video_components(self, components: List[str], output_path: str) -> bool:
        ...
        for component in components:
            cmd.extend(['-i', component])
        
        inputs = ''.join(f'[{i}:v:0][{i}:a:0]' for i in range(len(components)))
        filter_complex = f"{inputs}concat=n={len(components)}:v=1:a=1[v][a]"

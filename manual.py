...
class EnhancedContentGenerator:
    ...
    OUTPUT FORMAT:
    Return EXACTLY 15-25 clips as a JSON array. Each clip must be 15-90 seconds long.
    ...
    def _calculate_enhanced_trending_score(self, clip: Dict, trending_topics: List[TrendingTopic]) -> float:
        ...
        for topic in trending_topics:
            ...
            keyword_matches = sum(1 for keyword in topic.keywords if keyword.lower() in clip_text.lower())
            
            # Weight by search volume and recency
            topic_score = (keyword_matches * topic.search_volume * topic.recency_factor) / 100
    ...
    1. Title must create immediate curiosity gap
    2. Description must build urgency and provide value
    3. Hashtags must maximize discoverability
    4. All content must position Dr. Greenwall as THE authority
    5. Must address specific parent fears/concerns

    EXAMPLES OF VIRAL TITLES:
    - "The White Spot Test Every Parent Must Know"
    - "Stop Brushing Your Child's Teeth Like This"
    - "Dentist Reveals: Why MIH Gets Worse at Night"
    - "The 30-Second Check That Saves Your Child's Smile"
    - "What Chalky Teeth Really Mean (Parents Are Shocked)"

    EXAMPLES OF VIRAL DESCRIPTIONS:
    - "🚨 If your child has white spots on their teeth, WATCH THIS NOW. Dr. Greenwall reveals the mistake 8 out of 10 parents make that makes MIH worse. Don't wait - early action is everything! 👆 Save this for later"

    - "The moment I saw this, everything clicked 💡 Dr. Greenwall explains why regular toothpaste can damage MIH teeth. Parents, we've been doing it wrong! Tag a parent who needs to see this 👇"
...
class EnhancedVisualCreator:
    ...
    def create_branded_intro(self, title: str, trending_hook: str = "", duration: float = 5.0) -> Optional[str]:
        ...
        intro_text = trending_hook if trending_hook else f"Dr. Greenwall reveals: {clean_title}"
        ...
        video_filters.append('drawbox=x=0:y=800:w=1080:h=350:color=black@1:t=fill')
        
        # Add title text (handle 1 or 2 lines)
        if len(title_lines) == 1:
            # Single line - center vertically
            video_filters.append(f"drawtext=fontfile='C\\:/Windows/Fonts/arialbd.ttf':text='{title_lines[0]}':fontsize=44:fontcolor=white:x=(w-text_w)/2:y=880")

        else:
            # Two lines - position them appropriately
            video_filters.append(f"drawtext=fontfile='C\\:/Windows/Fonts/arialbd.ttf':text='{title_lines[0]}':fontsize=44:fontcolor=white:x=(w-text_w)/2:y=860")
            video_filters.append(f"drawtext=fontfile='C\\:/Windows/Fonts/arialbd.ttf':text='{title_lines[1]}':fontsize=44:fontcolor=white:x=(w-text_w)/2:y=900")
            video_filters.append(f"drawtext=fontfile='C\\:/Windows/Fonts/arialbd.ttf':text='{title_lines[2]}':fontsize=44:fontcolor=white:x=(w-text_w)/2:y=940")

        # Expert Insights badge
       
        video_filters.append("drawtext=fontfile='C\\:/Windows/Fonts/arial.ttf':text='EXPERT INSIGHTS':fontsize=20:fontcolor=white:x=(w-text_w)/2:y=1000")

        # Expert name
        video_filters.append(f"drawtext=fontfile='C\\:/Windows/Fonts/arialbd.ttf':text='{clean_expert_name}':fontsize=38:fontcolor=yellow:x=(w-text_w)/2:y=1030")

        # MIH Expert badge
        video_filters.append("drawtext=fontfile='C\\:/Windows/Fonts/arial.ttf':text='MIH EXPERT':fontsize=24:fontcolor=white:x=(w-text_w)/2:y=1080")
        ...
        try:
            # Set encoding for subprocess to handle Unicode properly
            import subprocess
    ...
    def create_branded_outro(self, duration: float = 3.5) -> Optional[str]:
        ...
        video_filters = [
            ...
            'drawbox=x=0:y=750:w=1080:h=450:color=black@1:t=fill',
            ...
            "drawtext=fontfile='C\\:/Windows/Fonts/arial.ttf':text='FOLLOW FOR MORE':fontsize=42:fontcolor=white:x=(w-text_w)/2:y=805",
            ...
            f"drawtext=fontfile='C\\:/Windows/Fonts/arial.ttf':text='{clean_expert_name}':fontsize=36:fontcolor=black:x=(w-text_w)/2:y=900",
            ...
            "drawtext=fontfile='C\\:/Windows/Fonts/arial.ttf':text='MIH EXPERT TIPS':fontsize=28:fontcolor=white:x=(w-text_w)/2:y=975",
            
            # Like and Subscribe with call-to-action styling
            'drawbox=x=250:y=1030:w=580:h=40:color=green@0.9:t=fill',
            "drawtext=fontfile='C\\:/Windows/Fonts/arial.ttf':text='LIKE and SUBSCRIBE':fontsize=26:fontcolor=white:x=(w-text_w)/2:y=1045",
            ...
            "drawtext=fontfile='C\\:/Windows/Fonts/arial.ttf':text='Transform Your Smile Today!':fontsize=24:fontcolor=white:x=(w-text_w)/2:y=1120",
            
            # Contact info or website
            "drawtext=fontfile='C\\:/Windows/Fonts/arial.ttf':text='lindagreenwall.co.uk':fontsize=22:fontcolor=cyan:x=(w-text_w)/2:y=1160",
        ...
        try:
            # Use safe subprocess call with proper encoding
            import subprocess
            result = subprocess.run(
                cmd,
                capture_output=True,
    ...
    #         # tts = edge_tts.Communicate(text=clean_text, voice="en-US-JennyNeural", rate="+0%", pitch="+10%")
    #         # tts.save(str(tts_file))
    ...
    def _create_animated_text_filter(self, text: str, fontsize: int, y: str, color: str, animation: str = 'fade') -> str:
        ...
        return f"drawtext=fontfile='C\\:/Windows/Fonts/arial.ttf':text='{safe_text}':fontsize={fontsize}:fontcolor={color}:x=(w-text_w)/2:y={y}:box=1:boxcolor=black@0.5"

    def _create_text_filter(self, text: str, fontsize: int, y: str, color: str) -> str:
        # Simplified - basic drawtext without fontweight
        safe_texvideo_filterst = self._escape_text_for_ffmpeg(text)
        return f"drawtext=fontfile='C\\:/Windows/Fonts/arial.ttf':text='{safe_text}':fontsize={fontsize}:fontcolor={color}:x=(w-text_w)/2:y={y}:box=1:boxcolor=black@0.5"
    ...
    def _escape_text_for_ffmpeg(self, text: str) -> str:
        ...
        # Remove or escape FFmpeg special characters
        text = text.replace('\\', '')
        text = text.replace(':', ' ')
        text = text.replace('[', '')
        text = text.replace(']', '')
...
class EnhancedVideoProcessor:
    ...
    f"drawtext=fontfile='C\\:/Windows/Fonts/arial.ttf':text='{self.config.expert_name}':fontcolor=white@0.7:fontsize=40:x=w-tw-25:y=25",
    f"drawtext=fontfile='C\\:/Windows/Fonts/arial.ttf':text='MIH EXPERT':fontcolor={self.config.brand_colors['accent']}:fontsize=32:x=25:y=25"
]
